...
def ask_ai(system_role: str, user_prompt: str, temperature: float = 1.0, model: str = "deepseek-ai/DeepSeek-V3"):
    ...
    四、世界观与逻辑
    1. 世界观中的核心规则必须自洽：
       - 能力系统（修为/超能力/科技）要有清晰限制，不能随剧情随便开挂。
       - 同一规则下的不同人物表现，要有强弱差异，但不能完全违背逻辑。
    2. 伏笔与回收：
       - 任何刻意描写的“奇怪细节”视为伏笔，后文必须有对应回收或解释。
       - 禁止在无意义之处堆砌花哨设定，占字数而不服务剧情。
    """
...
4. 描写上多用细节（视线、动作、环境噪音）来承载紧张感或情绪，而不是堆形容词。

请直接输出这一章的正文。
"""
raw_chapter = ask_ai("你是一名极其老练、擅长心理博弈与多线伏笔的长篇网文作者。", base_prompt, temperature=1.15)
...
请只输出新增部分正文，不要复述前文。
"""
new_part = ask_ai("你是在延续自己作品、非常在意逻辑和伏笔回收的作者。", cont_prompt, temperature=1.1)
                    if new_part:
                        combined = existing + "\n\n" + new_part

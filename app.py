        ...
        if st.button("🚀 一键生成整本书大纲 + 全部章节目录", use_container_width=True):
            if not protagonist.strip() or not world_setting.strip():
                ...
                with st.spinner("正在生成高强度大纲，请稍等……"):
                    ...
                    - 力量来源、阶梯划分、使用代价和限制。
                    - 社会结构 / 门派势力 / 阵营划分。
                    - 至少 1 条【禁忌规则】或【世界真相线索】。

                    三、主要角色阵容
                    - 主角：人设标签、外在目标、内在缺陷、人物弧光。
                    - 核心配角：各自站位（推主角？拖后腿？对照组？）。
                    - 长期对手 / 宿敌：前期目标 / 中期转折 / 后期立场变化。

                    四、故事阶段划分
                    - 例如：新手开局 / 小世界舞台 / 主战场 / 终局篇。
                    - 每个阶段给出涵盖的章节范围 + 阶段目标 + 必发生事件。

                    五、完整章节目录（最关键部分）
                    要求：
                    - 从【第1章】开始，按顺序列出，直到真正终局。
                    - 每一章必须给出：
                      · 章节号 + 章节名
                      · 2~4 句剧情简介
                      · 在括号中标注事件等级（小事件 / 中事件 / 大事件）
                      · 尽量体现冲突推进、信息揭露、人物关系变化。
                    - 特别注意：前 10 章要有足够密度的爽点和悬念。

                    六、长期伏笔与回收
                    - 列出 3~8 条关键伏笔。
                    - 标注【埋下章节】与【回收章节】，简要说明回收方式。

                    输出时，注意：
                    - 章节目录部分用清晰的行文格式，方便后续程序抽取，比如：
                      第1章 章节名 —— 简要剧情（事件级别：小事件）
                      第2章 ...
                    - 不要偷懒写“中间略”或“后文自行发挥”，要完整写到结局。
                    """
                    outline_full = ask_ai(
        ...
        if st.button("✍️ 高质量生成 / 重写本章（覆盖当前）", use_container_width=True):
            if not chapter_plan.strip():
                ...
                with st.spinner("正在根据大纲写这一章……"):
                    ...
                    【写作要求】：
                    1. 把这一章写成标准的网文章节，包含：
                       - 开场（尽量用一个具体场景或动作把人拉进来）
                       - 中段发展（冲突升级 / 信息揭露 / 关系变化）
                       - 尾声和小悬念（暂时收束当前矛盾，但为后文埋下钩子）。
                    2. 冲突优先使用“智斗、信息差、立场冲突”，人物要有各自动机和盘算。
                    3. 至少埋下 1~2 个可在后文回收的伏笔（用细节带过，不要高亮写“这是伏笔”）。
                    4. 文风偏向：{style}。
                    5. 字数目标：{word_target}，可以略有浮动，但剧情节点要完整。

                    请直接输出这一章的【正文内容】（不需要再解释你是如何设计的）。
                    """
                    text = ask_ai(
                    ...
                    if text:
                        ...
                        st.success("✅ 本章正文已生成，亮点摘要已提炼。右侧可查看和微调。")
        ...
        if st.button("➕ 高质量续写当前章节（在末尾接上）", use_container_width=True):
            ...
            if not base.strip():
                ...
                with st.spinner("正在在现有基础上续写……"):
                    ...
                    5. 续写长度参考：{word_target}，比数字更重要的是“情节节点完整”。

                    只输出【新增的续写正文】部分，不要重复前文。
                    """
                    add = ask_ai(
                        "你是在延续自己作品的作者，非常在意逻辑连续和伏笔回收。",
                        cont_prompt,
                        temperature=1.1
                    )
                    if add:
